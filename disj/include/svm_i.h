...
class SVM_I : public MLalgo //SVM
{
	...
	public:
		...
		SVM_I(int type = 0, void (*f) (const char*) = NULL, int size = 1000000) : max_size(size) {
			...
			etimes = 0;
			poly_num = 0;
			positive_mapped_data = new MState[max_size];
			positive_label = new int[max_size];
...
#endif
			if (positive_mapped_data != NULL) delete []positive_mapped_data;
#ifdef __PRT_DEBUG
			std::cout << "SVM_I deleted positive_mapped_data\n";
#endif
			if (negative_mapped_data != NULL) delete []negative_mapped_data;
#ifdef __PRT_DEBUG
			std::cout << "SVM_I deleted negative_mapped_data\n";
#endif
			if (positive_label!= NULL) delete []positive_label;
#ifdef __PRT_DEBUG
			std::cout << "SVM_I deleted positive_label\n";
#endif
		...
		int makeTrainingSet(int b) {
			assert (first_positive == false);
			for (int i = 0; i < negative_size; i++) {
				data[i] = negative_mapped_data[i];
				label[i] = -1;
			}
			problem.l = negative_size;
			branch = b;

#ifdef __DS_ENABLED
			problem.np = positive_size;
			problem.nn = negative_size;
#endif
		...
		void mapData(States* gsets, int& pre_psize, int& pre_nsize) {
			...
			/*#ifdef __TRAINSET_SIZE_RESTRICTED
			  ...
			  mappingData(gsets[POSITIVE].values[pstart + i], positive_mapped_data[i], 4);
			  positive_label[i] = gsets[POSITIVE].labels[pstart+i];
...
*/
			...
			for (int i = 0; i < cur_psize - pre_psize; i++) {
				mappingData(gsets[POSITIVE].values[pre_psize + i], positive_mapped_data[cur_index + i], 4);
				positive_label[i] = gsets[POSITIVE].labels[pre_psize+i];
			}
		...
		int train() {
			if (problem.y == NULL || problem.x == NULL || positive_mapped_data == NULL || negative_mapped_data == NULL) return -1;

#ifdef __PRT_SVM_I
#if 0
			std::cout << "===============THE WHOLE TRAINING SET:=============\nPOSITIVE: ";
			for (int i = 0; i < positive_size; i++) {
				std::cout << GREEN << i << "[" << positive_label[i] << "](" << positive_mapped_data[i][0];
				for (int j = 1; j < Nv; j++)
					std::cout << "," << positive_mapped_data[i][j];
				std::cout << ")   " << NORMAL;
			}
			std::cout << "\nNEGATIVE:";
			for (int i = 0; i < negative_size; i++) {
				std::cout << RED << i << "(" << negative_mapped_data[i][0];
				for (int j = 1; j < Nv; j++)
			...
			for (cl.size = 0; cl.size < cl.max_size;) {
				...
				std::cout << YELLOW << "MISCLASSIFIED INDEX[" << misidx << "]" << NORMAL;
#endif
				if (stepTrain(misidx) < 0) {
					std::cout << "Can not classify state [index" << misidx << "](" << positive_mapped_data[misidx][0];
					for (int i = 1; i < Nv; i++) {
						std::cout << ", " << positive_mapped_data[misidx][i];
					}
					std::cout << ") against other " << problem.l << " negative states.\n";
					return -1;
				}
			}
		...
		double checkTrainingSet()
		{
			...
			for (int i = 0; i < positive_size; i++) {
				/*
				   ...
				   */
				if (positive_label[i] != branch) continue;
				total++;
				int presult = predict(positive_mapped_data[i]);
				if (presult == 0) {
					std::cout << "predict error in checkTrainingSet function.\n";
					return 0;
				}
				if (presult != 1) std::cout << RED << "positive<" << i << "> predict fails.\n" << NORMAL; 
				pass += (presult == 1) ? 1 : 0;
			}
			for (int i = 0; i < negative_size; i++) {
				/*
		...
		double partialCheckTrainingSet(int removed_cl)
		{
			...
			for (int i = 0; i < positive_size; i++) {
				if (positive_label[i] != branch) continue;
				total++;
				int presult = partialPredict(positive_mapped_data[i], removed_cl);
				if (presult == 0) {
		...
		int stepTrain(int positive_index) {
			if ((positive_index < 0) || (positive_index >= positive_size))
				return -1;
			//std::cout << problem;
			label[problem.l] = 1;
			data[problem.l] = positive_mapped_data[positive_index];
			problem.l++;
		...
		int getMisclassified(int& idx) {
			...
			if (cl.size == 0) {
				for (idx = 0; idx < positive_size; idx++) {
					if (positive_label[idx] == branch)
						break;
				}
				if (idx >= positive_size)
					return -1;
				return 0;
			}
...
#endif
			for (int i = 0; i < positive_size; i++) {
				int k = (i + start) % positive_size;
				if (positive_label[k] != branch) continue;
				if (predict(positive_mapped_data[k]) < 0) {
#ifdef __PRT_SVM_I
					std::cout << "\n [FAIL] @" << k << ": (" << positive_mapped_data[k][0];
					for (int j = 1; j < Nv; j++)
						std::cout << "," << positive_mapped_data[k][j];
